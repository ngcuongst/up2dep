...
public class LibUsageViewManager extends UsageViewManager implements Disposable {
    private static final Logger LOG = Logger.getInstance(LibUsageViewManager.class);
    private final Project myProject;
    private static final Key<UsageView> USAGE_VIEW_KEY = Key.create("USAGE_VIEW");
    private static LibUsageViewManager instance = null;

    public static LibUsageViewManager getInstance(@NotNull Project project) {
        if (instance == null) {
            instance = new LibUsageViewManager(project);
        }

        return instance;
    }

    public LibUsageViewManager(@NotNull Project project) {
        myProject = project;
    ...
    @Override
    @NotNull
    public LibUsageView createUsageView(@NotNull UsageTarget[] targets,
                                        @NotNull Usage[] usages,
                                        @NotNull UsageViewPresentation presentation,
                                        Factory<UsageSearcher> usageSearcherFactory) {
        LibUsageView usageView = new LibUsageView(myProject, presentation, targets, usageSearcherFactory);
    ...
    public UsageView showUsages(@NotNull UsageTarget[] searchedFor,
        ...
        addContent((LibUsageView) usageView, presentation);
        showToolWindow(true);
        UIUtil.invokeLaterIfNeeded(() -> {
            if (!((LibUsageView) usageView).isDisposed()) {
                ((LibUsageView) usageView).expandRoot();
            }
        });
        return usageView;
    }
    ...
    void addContent(@NotNull LibUsageView usageView, @NotNull UsageViewPresentation presentation) {
    ...
    private UsageView doSearchAndShow(@NotNull final UsageTarget[] searchFor,
        ...
        final AtomicReference<LibUsageView> usageViewRef = new AtomicReference<>();
        ...
        Task.Backgroundable task = new Task.Backgroundable(myProject, getProgressTitle(presentation), true, new SearchInBackgroundOption()) {
            ...
            public void run(@NotNull final ProgressIndicator indicator) {
                new SearchForUsagesRunnable(LibUsageViewManager.this, LibUsageViewManager.this.myProject, usageViewRef, presentation, searchFor, searcherFactory,
            ...
            public NotificationInfo getNotificationInfo() {
                LibUsageView usageView = usageViewRef.get();
    ...
    protected static void appendUsages(@NotNull final Usage[] foundUsages, @NotNull final LibUsageView usageView) {
    ...
    public static void showTooManyUsagesWarningLater(@NotNull final Project project,
                                                     ...
                                                     @Nullable final LibUsageView usageView) {
        UIUtil.invokeLaterIfNeeded(() -> {
