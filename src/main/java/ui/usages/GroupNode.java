...
public class GroupNode extends Node implements Navigatable, Comparable<GroupNode> {
    private static final NodeComparator COMPARATOR = new GroupNode.NodeComparator();
    ...
    @NotNull
    UsageNode addUsage(@NotNull Usage usage, @NotNull Consumer<Node> edtInsertedUnderQueue, boolean filterDuplicateLines, PsiElement psiElement) {
    ...
    private static class NodeComparator implements Comparator<DefaultMutableTreeNode> {
        ...
        private static GroupNode.NodeComparator.ClassIndex getClassIndex(DefaultMutableTreeNode node) {
            if (node instanceof UsageNode) return NodeComparator.ClassIndex.USAGE;
        ...
        public int compare(DefaultMutableTreeNode n1, DefaultMutableTreeNode n2) {
            ...
            NodeComparator.ClassIndex classIdx2 = getClassIndex(n2);
            if (classIdx1 != classIdx2) return classIdx1.compareTo(classIdx2);
            if (classIdx1 == NodeComparator.ClassIndex.GROUP)
                return ((GroupNode) n1).compareTo((GroupNode) n2);
            if (classIdx1 == NodeComparator.ClassIndex.USAGE)
